...
void cPlayer::Move()
{
    ...
    if((fTime < GAME_TIME) && (fTime > 0.0f))
    {
        ...
        m_vVelocity += vMove * (m_fSpeed * PLAYER_ACCELERATION * Core::System->GetTime());
        if(!m_vVelocity.IsNull()) m_vVelocity *= POW(1.0f - PLAYER_BRAKE * (1.0f/60.0f), Core::System->GetTime() * 60.0f);
